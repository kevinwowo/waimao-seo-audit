...
报告底部固定会显示 `contact.json` 里写的微信/邮箱/网站。用户拿到报告读完,有问题会自然找到你——这是个天然的获客钩子。

如果你是外贸独立站从业者,想用这个工具拓客:fork 一份、改 `contact.json` 为你自己的信息、按 README 部署或直接分发跑好的报告。**MIT 协议,可商用,不必署名原作者**。

如果你的客户技术能力有限,你可以替他们跑(只需要他们告诉你网址),把报告 PDF 发回去。
